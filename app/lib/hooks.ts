...
export type CostCatalog = {
  ...
  default_form: string;
  /** 레거시 4단 시세 배율 — 시도·구를 고르지 않았을 때의 폴백 */
  regions: Array<{ name: string; pct: number; desc: string }>;
  default_region: string;
  /** 데이터 기반 거주지 선택지 (KOSIS 시도 × 서울 자치구) */
  region_coefficients: RegionCatalog;
 ...
 * "왜 이 금액인가요?" 설명.
 *
 * 시트가 열려 있는 동안만 마운트되므로 호출도 그때만 일어난다. 백엔드가 LLM 실패 시
...
export function useDontworryExplain(
  ...
  useEffect(() => {
    ...
    const cached = explainMemo.get(key);
    if (cached) {
