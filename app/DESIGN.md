...
타일에 배율·금액 같은 보조 수치를 붙일 때는 제목 아래 한 줄로만 두고, `--muted` 계열 색과 10–11px 로 낮춘다. 제목과 보조 수치의 기준선은 그룹 안에서 어긋나지 않게 한다.

## Tiered choice tiles

상위 선택에 따라 하위 선택이 열리는 2단 구조는 같은 타일 패턴을 이어 쓴다. 하위 그룹은 들여쓰기나 새 카드로 감싸지 않고, 1px 점선 상단 구분선과 짧은 안내 문장으로 층위를 알린다. 항목이 많아 좁혀야 할 때는 열 수만 늘리고(2열 → 3열) 타일의 최소 높이는 44px 이상을 유지한다. 하위 선택은 선택 해제가 가능해야 하며, 상위 선택이 바뀌면 하위 선택은 버린다. 하위 그룹이 필수가 아닐 때는 제목에 `(선택)`을 붙인다.

## Explain triggers and sheets

계산된 숫자의 근거를 따로 보여줄 때는 그 숫자 **바로 옆**에 `Info` 아이콘 버튼을 둔다. 아이콘은 17px, 색은 `--muted`, hover·focus 에서만 메인 그린으로 바뀐다. 아이콘이 작아도 터치 영역은 44×44px 를 유지하고, 접근 가능한 이름을 붙인다(`이 금액이 나온 이유 보기`). 숫자와 같은 줄에 두어 무엇에 대한 설명인지 분명히 한다.

설명은 약관 팝업과 같은 바텀시트를 쓰되 높이를 내용에 맞춘다(`height: auto`, `max-height: 70svh`). 두세 문장짜리 설명에 화면 높이를 다 쓰지 않는다. 본문은 15px·행간 1.7 로 읽기 편하게 두고, 하단에는 출처 각주 규칙을 그대로 적용한다.

기다림이 있으면 스피너 대신 문장 모양 스켈레톤을 쓴다 — 곧 들어올 것이 문장이라는 걸 형태로 알린다. 마지막 줄은 62% 폭으로 줄여 문단처럼 보이게 한다. `prefers-reduced-motion` 에서는 shimmer 를 끈다. 캐시된 응답이면 스켈레톤이 거의 보이지 않는 것이 정상이다.

## Source footnotes

공개데이터·통계에서 끌어온 숫자를 보여주는 화면은 맨 아래에 출처와 갱신 시점을 한 문장으로 밝힌다. `Info` 아이콘 14px 을 문장 앞에 두고, 10px `--muted` 로 낮춰 본문과 경쟁하지 않게 한다. 배경·테두리를 주지 않는다. 데이터를 불러오지 못해 기본값으로 계산한 경우에도 같은 자리에서 그 사실을 알리고 숨기지 않는다.
