...
class Component extends React.Component {
  ...
  submitEnroll = async () => {
    ...
    // 청약서 → 스코어카드 요율 입력 매핑
    const income_band =
      a.income.includes('800만원') ? 5 :
      a.income.includes('650~800') ? 4 :
      a.income.includes('550~650') ? 3 :
      a.income.includes('300~550') ? 2 : 1;
    const retake_intent = 3;
  ...
  tierFallback = [
    { tier: '라이트',   form: '독학재수',   cover_mild: 2100000,  cover_sev: 4200000,  monthly_premium: 2098 },
    { tier: '스탠다드', form: '단과 통학',  cover_mild: 4200000,  cover_sev: 8400000,  monthly_premium: 3842 },
    { tier: '플러스',   form: '재종합학원', cover_mild: 7020000,  cover_sev: 14030000, monthly_premium: 6179 },
    { tier: '프리미엄', form: '기숙학원',   cover_mild: 10500000, cover_sev: 21000000, monthly_premium: 9073 },
  ];
  ...
  policyUrl = 'policy/jaesoo-policy.html';
            ...
            <div style={S("background:#fff;display:flex;flex-direction:column;width:100%;min-height:100dvh")}>
              ...
              <div style={S("max-width:900px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                ...
                <div style={S("font-size:11.5px;color:#9AA0AB;margin-top:14px;line-height:1.7;word-break:keep-all")}>
                  ...
                  <a href={vm.policyUrl + '#jehan'} target="_blank" rel="noreferrer" style={S("color:#0C6742;font-weight:700")}>전체 지급제한사항 보기 ↗</a>
                </div>
              </div>
                    ...
                    <div style={S("padding:14px 15px;display:flex;flex-direction:column;gap:16px")}>
                      ...
                      <div>
                        ...
                        <div style={S("display:flex;gap:6px;flex-wrap:wrap")}>
                          {['1단계(읍·면지역)','2단계(중소도시)','3단계(대도시)','4단계(특별시급)'].map((o,oi)=>(<div key={oi} onClick={()=>vm.setApply({region:o})} style={S(`font-size:10px;font-weight:600;padding:7px 11px;border-radius:8px;cursor:pointer;border:1.5px solid ${vm.apply.region===o?'#0B7A4A':'#DDD'};background:${vm.apply.region===o?'#0B7A4A':'#fff'};color:${vm.apply.region===o?'#fff':'#555'}`)}>{o}</div>))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        {[
                          ['가구소득 (월평균)','income',['1분위(약 300만원 미만)','2분위(약 300~550만원)','3분위(약 550~650만원)','4분위(약 650~800만원)','5분위(약 800만원 이상)']],
                          ['거주지역 (학원밀집도지수)','region',['1단계(읍·면지역)','2단계(중소도시)','3단계(대도시)','4단계(특별시급)']],
                          ['월교육비 (피보험자 1인 기준)','edu_cost',['1분위(약 30만원 미만)','2분위(약 30~52만원)','3분위(약 52~75만원)','4분위(약 75~120만원)','5분위(약 120만원 이상)']]
                        ].map((q,qi)=>(
                          <React.Fragment key={qi}>
                            <div style={S("margin-top:9px")}>
