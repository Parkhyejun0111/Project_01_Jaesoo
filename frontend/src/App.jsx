...
class Component extends React.Component {
  state = {
    ...
    tiers: [], expandedTier: null, selectedPass: 0,
  ...
  // 인강 패스 상품 — 랜딩 그리드와 결제 화면이 같은 정의를 공유한다.
  passDefs = [
    { tag: 'BEST', tagC: '#2B4FE8', title: '올인원 메가패스', sub: '국·수·영·탐 전 강좌 · 12개월 무제한',
      detail: '2026 올인원 메가패스 · 전 강좌 무제한', meta: '김서준 · 이도현 · 박지훈 등 · 12개월',
      was: 460000, price: 396000, hot: true },
    { tag: '단과', tagC: '#0B8F58', title: '단과 집중 패스', sub: '약점 과목 단과 3강좌 자유 선택',
      detail: '2026 단과 집중 패스 · 3강좌 자유 선택', meta: '과목별 대표 강사 · 6개월',
      was: 240000, price: 198000, hot: false },
    { tag: '재종', tagC: '#E0821A', title: '재종 통학 패스', sub: '재수종합 커리큘럼 + 담임관리',
      detail: '2026 재종 통학 패스 · 재수종합 커리큘럼', meta: '담임관리 + 주간 모의고사 · 12개월',
      was: 1980000, price: 1670000, hot: false },
    { tag: '기숙', tagC: '#B03A5B', title: '프리미엄 기숙 패스', sub: '전일제 기숙 + 밀착 컨설팅',
      detail: '2026 프리미엄 기숙 패스 · 전일제 기숙', meta: '밀착 컨설팅 + 생활관리 · 12개월',
      was: 2900000, price: 2500000, hot: false },
  ];
  ...
  renderVals() {
    ...
    return {
      ...
      passes: this.passDefs,
      selectedPass: this.passDefs[s.selectedPass] || this.passDefs[0],
      pickPass: (i) => this.setState({ selectedPass: i, entry: 'pay' }),
      mode: s.mode,
      navApp: () => { try { window.history.pushState({}, '', '/app'); } catch (e) {} this.setState({ mode: 'app', entry: 'login' }); },
            ...
            <div style={S("background:#fff;display:flex;flex-direction:column;width:100%")}>
              ...
              <div style={S("max-width:1160px;margin:0 auto;padding:56px 20px 20px;width:100%;box-sizing:border-box")}>
                ...
                <div style={S("display:flex;gap:18px;margin-top:26px;flex-wrap:wrap")}>
                  {(vm.passes || []).map((p,pi)=>(<React.Fragment key={pi}>
                    <div style={S(`flex:1;min-width:250px;border:1px solid ${p.hot?'#B9CCFF':'#E7EAEF'};border-radius:20px;overflow:hidden;background:#fff;box-shadow:${p.hot?'0 14px 34px rgba(43,79,232,0.14)':'0 6px 18px rgba(20,25,40,0.04)'};display:flex;flex-direction:column`)}>
                      ...
                      <div style={S("padding:16px 18px;display:flex;flex-direction:column;gap:12px;flex:1")}>
                        <div>
                          <div style={S("font-size:11.5px;color:#B4B9C2;text-decoration:line-through")}>{p.was.toLocaleString('ko-KR')}원</div>
                          <div style={S("font-size:23px;font-weight:900;color:#16181D")}>{p.price.toLocaleString('ko-KR')}<span style={S("font-size:13px;font-weight:700")}>원</span></div>
                        </div>
                        <div style={S("margin-top:auto;background:#2B4FE8;color:#fff;font-size:13.5px;font-weight:800;border-radius:12px;height:44px;display:flex;align-items:center;justify-content:center;cursor:pointer")} onClick={() => vm.pickPass(pi)}>수강신청 ›</div>
                      </div>
                    </div>
                  </React.Fragment>))}
                </div>
              ...
              <div style={S(['apply','done'].includes(vm.entry) ? "padding:24px 30px 44px;display:flex;flex-direction:column;gap:14px;flex:1" : "padding:16px;display:flex;flex-direction:column;gap:12px")}>
                {(vm.entry === 'pay') && (<>
                  <div style={S("background:#fff;border:1px solid #E6E6E6;border-radius:20px;padding:15px")}>
                    ...
                    <div style={S("font-size:14px;font-weight:800;color:#111;margin-top:8px;line-height:1.4")}>{vm.selectedPass.detail}</div>
                    <div style={S("font-size:10.5px;color:#888;margin-top:3px")}>{vm.selectedPass.meta}</div>
                    <div style={S("display:flex;justify-content:space-between;align-items:baseline;margin-top:12px;border-top:1px solid #F0F0F0;padding-top:12px")}>
                      ...
                      <span style={S("font-size:17px;font-weight:800;color:#111")}>{vm.selectedPass.price.toLocaleString('ko-KR')}원</span>
                    </div>
                  </div>
