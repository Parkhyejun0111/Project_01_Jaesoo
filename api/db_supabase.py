...
create table if not exists jaesoo_explain_cache (      -- 돈워리 "왜 이 금액인가요?" 설명 문구 캐시
  cache_key   text primary key,   -- user_id:재수유형:시도:구:계수갱신시점
  answer      text not null,
  expires_at  timestamptz not null,
  created_at  timestamptz default now()
);
create index if not exists idx_jaesoo_explain_cache_expiry on jaesoo_explain_cache(expires_at);
...
-- 돈워리 지역계수용 거주지. 요율의 region(지역규모)과는 다른 개념이므로 별 컬럼으로 둔다.
--   residence_sido : KOSIS 시도 표기의 짧은 이름 ('서울', '경기', '충북' …)
...
_ENROLL_DEFAULTS = {
    ...
    "terms_agreed": False,
    # 돈워리 지역계수용 거주지 (요율과 무관).
    "residence_sido": None, "residence_gu": None,
}
        ...
        cur.execute(
            """insert into jaesoo_enrollments
               ...
               values (%(student_id)s, %(tier)s, %(region)s, %(academy_density_index)s,
                       ...
                       %(residence_sido)s, %(residence_gu)s)
               returning id""",
            params,
        )
...
def get_residence(student_id: str, conn=None) -> tuple[str | None, str | None]:
    """가입 시 저장한 돈워리 거주지 (시도, 구). 없으면 (None, None).

    요율의 region(지역규모)과는 다른 컬럼이다 — 섞어 쓰면 안 된다.
    """
    with _use(conn) as conn, conn.cursor() as cur:
        cur.execute(
            "select residence_sido, residence_gu from jaesoo_enrollments "
            "where student_id = %s order by created_at desc limit 1",
            (student_id,),
        )
        row = cur.fetchone()
...
# ── 돈워리 설명 문구 캐시 ────────────────────────────────────────────────────
def get_explain_cache(cache_key: str) -> tuple[str, float] | None:
    """(문구, 만료시각 epoch) 또는 None. 만료된 행은 미스로 취급한다."""
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            "select answer, extract(epoch from expires_at) from jaesoo_explain_cache "
...
def set_explain_cache(cache_key: str, answer: str, ttl_seconds: int) -> None:
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            "insert into jaesoo_explain_cache (cache_key, answer, expires_at) "
            "values (%s, %s, now() + make_interval(secs => %s)) "
            "on conflict (cache_key) do update set answer = excluded.answer, "
            "expires_at = excluded.expires_at, created_at = now()",
            (cache_key, answer, ttl_seconds),
        )
        conn.commit()


def purge_expired_explain_cache() -> int:
    """만료 행 정리 — 지역계수 월간 배치와 같은 주기로 불러도 된다."""
    with _connect() as conn, conn.cursor() as cur:
        cur.execute("delete from jaesoo_explain_cache where expires_at <= now()")
