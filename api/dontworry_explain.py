...
# ── 모델 설정 ───────────────────────────────────────────────────────────────
#   기본값은 현행 Sonnet. 한두 문장 서술 작업이므로 Sonnet 급이면 충분하다.
...
- 반드시 아래 내용을 이 순서로 담는다. 화면에는 최종 금액만 크게 떠 있고
  ...
  5) (학군지여부가 true인 경우) 학군지라 학원비 자체가 높다는 맥락
- 한 문장으로 답한다. 위 내용이 도저히 한 문장에 안 들어갈 때만 두 문장까지 쓴다.
  화면에서 작은 말풍선 하나로 보여주기 때문에 길면 잘린다.
- 친근한 존댓말, 숫자는 원 단위가 아니라 '만원' 단위로 자연스럽게.
...
def _allowed_numbers(breakdown: dict) -> set[str]:
    ...
    def add(value) -> None:
        ...
        if isinstance(value, (int, float)):
            ...
            # 소수 둘째 자리까지 허용하는 이유: 최종지역계수 2.0651 을 "약 2.07배"로
            # 옮겨 적는 건 환각이 아니라 정당한 반올림인데, 1자리(2.1)·정수(2)만
            # 허용하던 때에는 이게 근거 없는 숫자로 걸려 학군지 설명이 매번
            # 폴백으로 떨어졌다.
            allowed.add(_norm_number(round(float(value), 2)))
...
def _generate(breakdown: dict, user_id: str, 재수유형: str, *, client=None) -> str:
    """툴 결과를 자연어 한두 문장으로. 실패하면 예외를 올린다 (호출부가 폴백).
