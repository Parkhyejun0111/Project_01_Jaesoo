...
export class ApiClient {
  ...
  /** 지역계수 표 + 출처·갱신시점. sido 를 주면 그 지역의 분해도 함께 온다. */
  regionCoefficients({ sido, gu } = {}) {
    return this.request("/api/region-coefficients", { params: { sido, gu } });
  }
  /** "왜 이 금액인가요?" 설명. 같은 조합은 서버 캐시에서 즉시 온다. */
  dontworryExplain({ userId, 재수유형, sido, gu, signal }) {
    return this.request("/api/dontworry/explain", {
      method: "POST",
      body: { user_id: userId, 재수유형, sido: sido || null, gu: gu || null },
      signal,
    });
  }
  ...
  /** 청구 1단계가 고를 등록 카드 목록. 활성 카드가 먼저, 그다음 최근 갱신 순. */
  userCards(userId, { activeOnly, limit } = {}) {
