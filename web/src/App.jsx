...
const REGION_BANDS = [
  { label: '읍·면 지역', value: '읍면지역', density: 1, desc: '군 지역' },
  { label: '중소도시', value: '중소도시', density: 2, desc: '그 외 시 지역' },
  { label: '광역시', value: '대도시', density: 3, desc: '부산·대구·인천·광주·대전·울산' },
  { label: '특별시', value: '특별시', density: 4, desc: '서울·세종' },
];
...
class Component extends React.Component {
  resetApplyForm = () => ({
    ...
    p_name: '', p_birth: '',
    g_name: '', g_phone: '',
    q_grade: '', q_direction: '', q_wait: false, q_data: '',
    d2: '', d3: '', d4: '', d5: '',
  ...
  uiIcon = (kind, size = 22, stroke = 'currentColor', sw = 1.6) => {
    const paths = {
      ...
      heavyFace: (<><circle cx="12" cy="12.5" r="8.8" /><path d="M7.8 9.9l2.5 .7" /><path d="M16.2 9.9l-2.5 .7" /><circle cx="9.3" cy="13" r="1.22" fill={stroke} stroke="none" /><circle cx="14.7" cy="13" r="1.22" fill={stroke} stroke="none" /><path d="M8.9 16.8q3.1-2.2 6.2 0" /><rect x="7.1" y="4.7" width="9.8" height="2.7" rx="1.2" /></>),
      moneyFace: (<><circle cx="12" cy="12.4" r="8.8" /><path d="M8 9.8h2.6" /><path d="M13.4 9.8h2.6" /><circle cx="9.3" cy="12.9" r="1.18" fill={stroke} stroke="none" /><circle cx="14.7" cy="12.9" r="1.18" fill={stroke} stroke="none" /><path d="M9.2 16.4h5.6" /><path d="M6.3 5.6h2.4M17.7 5.6h-2.4" /></>),
    };
    return (
      <svg viewBox="0 0 24 24" width={size} height={size} fill="none"
  ...
  tierScale = [
    { ink: '#3E9E74', line: '#BEE3CE', tint: 'rgba(62,158,116,0.05)',  btn: 'linear-gradient(135deg,#3E9E74,#57B189)' },
    { ink: '#1F8659', line: '#9FD3B8', tint: 'rgba(31,134,89,0.065)',  btn: 'linear-gradient(135deg,#1F8659,#35A272)' },
    { ink: '#0C6742', line: '#84C2A3', tint: 'rgba(12,103,66,0.08)',   btn: 'linear-gradient(135deg,#0C6742,#1B8258)' },
    { ink: '#00462A', line: '#6BB08F', tint: 'rgba(0,70,42,0.095)',    btn: 'linear-gradient(135deg,#00462A,#0A5F3C)' },
  ];
  ...
  flowSteps = [
    { icon: 'cart', title: '수강신청 시 가입', desc: '인강 결제 단계에서 보험을 옵션으로 함께 선택해요. 별도 가입 절차가 없어요.' },
  ...
  applyPrep = [
    ...
    { no: '2', title: '가입자격 확인', desc: '현재 학년, 대기기간 안내 확인, 성적자료 제출 동의. 성적자료 제출에 동의하지 않으면 가입이 불가합니다.' },
    { no: '3', title: '고지사항', desc: '재수 계획 유무, 학업 중단·휴학·유급 이력, 학업에 지장을 주는 질병·장애, 타사 유사보험, 수능 외 진로 계획 등 5개 문항.' },
    ...
    { no: '7', title: '전자서명 또는 자필서명', desc: '계약자·피보험자 본인이 서명하며, 미성년자인 경우 법정대리인의 서명이 추가로 필요합니다.' },
  ];
  ...
  faqs = [
    ...
    { q: '보험금은 어떻게 지급되나요?', a: '보통약관은 경증·중증으로 나누어 보험가입금액을 지급하며, 최초 1회한이고 경증과 중증은 중복 지급하지 않습니다. 스탠다드 기준 경증 420만원, 중증 840만원이 예시입니다. 별도로 재수학원비 지원 특약(실손형)을 선택하면 실제 납부한 수강료를 영수증 기준 실비로 연간 한도 내에서 보상받을 수 있어요.' },
              ...
              >
                ...
                <div key="hero1" style={S("padding:clamp(34px,4.6vw,52px) clamp(24px,6vw,64px) clamp(56px,6vw,64px);display:flex;align-items:center;gap:clamp(24px,4vw,40px);flex-wrap:wrap;animation:heroIn .9s cubic-bezier(.16,.84,.44,1) both")}>
                  <div style={S("flex:1 1 300px;min-width:0")}>
                    ...
                    {/* 인강 결제 화면 진입점. 임베디드 가입은 이 결제 단계에서 일어난다 —
                        기존에는 이 버튼이 없어 결제 화면에 도달할 수 없었다. */}
                    <div className="hov-btn" style={S("width:min(300px,100%);height:60px;display:flex;align-items:center;justify-content:center;box-sizing:border-box;margin-top:26px;background:linear-gradient(135deg,#2B4FE8,#4E7CFF);color:#fff;font-size:17px;font-weight:900;border-radius:16px;cursor:pointer;box-shadow:0 14px 26px rgba(43,79,232,0.28)")}
                         role="button" tabIndex={0}
                         onClick={() => vm.entryGo('pay', 'landing')}
                         onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') vm.entryGo('pay', 'landing'); }}>메가패스 수강신청 →</div>
                  </div>
          ...
          {((vm.entry === 'insurance' || (vm.payBackEntry === 'insurance' && ['terms'].includes(vm.entry)))) && (<>
            <div style={S("background:#fff;display:flex;flex-direction:column;width:100%;min-height:100dvh")}>
              ...
              <div style={S("width:100%;background:#fff;border-bottom:1px solid #ECEEF1;position:sticky;top:0;z-index:8")}>
                <div style={S("max-width:1160px;margin:0 auto;min-height:60px;padding:10px 20px;display:flex;align-items:center;gap:14px;flex-wrap:wrap")}>
                  ...
                  <div style={S("flex:1;min-width:0;display:flex;align-items:center")}>
                    <span style={S("font-size:15px;font-weight:900;color:#00462A;letter-spacing:-0.5px;white-space:nowrap")}>재수없수 학습성취 보장보험</span>
                  </div>
                </div>
              </div>

              {/* 히어로 */}
              <div style={S("width:100%;background:linear-gradient(160deg,#EAF7EC 0%,#F4FBF5 100%)")}>
                <div style={S("max-width:820px;margin:0 auto;padding:clamp(36px,5vw,56px) 20px;text-align:center")}>
                  <div style={S("display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:800;color:#0B8F58;background:#fff;border:1px solid #BEE3CE;border-radius:20px;padding:6px 13px")}>인강 임베디드 보험</div>
              ...
              <div style={S("width:100%;background:#fff;display:flex;justify-content:center;padding:clamp(22px,3.4vw,38px) 20px")}>
                <div style={S("width:100%;max-width:640px;background:#DCECE4;border-radius:clamp(20px,2.4vw,28px);padding:clamp(34px,4.6vw,48px) clamp(22px,3.6vw,40px);box-sizing:border-box")}>
                  <div style={S("text-align:center")}>
                    <div style={S("display:inline-block;font-size:12px;font-weight:800;color:#0C6742;background:#fff;border-radius:20px;padding:6px 14px;box-shadow:0 2px 8px rgba(0,70,42,0.05)")}>재수, 마음보다 먼저 계산기를 두드리게 됩니다</div>
                    ...
                    <div style={S("font-size:13px;color:#5C7268;margin-top:12px;line-height:1.7;word-break:keep-all")}>실제로 많이 하시는 이야기들이에요.</div>
                  </div>
                  ...
                  <div style={S("display:flex;flex-direction:column;gap:clamp(9px,1.2vw,12px);margin-top:clamp(20px,2.6vw,26px)")}>
                    {[
                      ...
                      { side: 'right', who: '고2 학부모', icon: 'swingFace', parts: [{t:'성적이 '},{t:'오르락내리락',fx:'wave'},{t:'하는데, 그걸 딱 하루로 판단한다는 게 불안해요.'}] },
                      ...
                      return (
                        <React.Fragment key={bi}>
                          <div style={S(`display:flex;align-items:flex-start;gap:9px;justify-content:${isLeft ? 'flex-start' : 'flex-end'}`)}>
                            {isLeft && (<span style={S("flex:none;width:48px;height:48px;border-radius:50%;background:#F2FBF6;border:1.5px solid #BEE3CE;display:flex;align-items:center;justify-content:center;margin-top:0;box-shadow:0 4px 10px rgba(0,70,42,0.08)")}>{this.uiIcon(b.icon, 32, '#06734A', 2.05)}</span>)}
                  ...
                  <div style={S("display:flex;flex-direction:column;align-items:center;gap:5px;margin-top:clamp(18px,2.4vw,24px)")}>
                    ...
                    <div style={S("font-size:13px;font-weight:800;color:#0C6742;margin-top:11px;text-align:center;line-height:1.7;word-break:keep-all")}>그래서 먼저, 재수에 실제로 얼마가 드는지부터 정리했습니다.</div>
                  </div>
                </div>
              </div>
              ...
              <div style={S("max-width:900px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>WHY IT MATTERS</div>
                  ...
                  <div style={S("font-size:13.5px;color:#7A808B;margin-top:10px;line-height:1.7;word-break:keep-all")}>재수 형태에 따라 1년에 {vm.costBars.minLabel}에서 {vm.costBars.maxLabel}까지 듭니다.<br/>재수없수는 이 비용의 <b style={S("color:#0C6742")}>70%</b>를 보장합니다.</div>
                </div>
              ...
              <div style={S("max-width:1160px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>HOW IT WORKS</div>
                  ...
                  {vm.flowSteps.map((f, fi) => (
                    <React.Fragment key={fi}>
                      <div className="hov-lift" style={S("flex:1 1 210px;min-width:0;border:1px solid #E7EAEF;border-radius:18px;padding:22px 20px;background:#fff;display:flex;flex-direction:column")}>
                        <span style={S("align-self:flex-start;font-size:11px;font-weight:900;color:#0B8F58;background:rgba(11,143,88,0.09);border-radius:20px;padding:5px 12px")}>STEP {fi + 1}</span>
              ...
              <div style={S("max-width:1160px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>COVERAGE PLANS</div>
                ...
                <div style={S("margin-top:26px;overflow-x:auto;border:1px solid #E7EAEF;border-radius:20px;background:#fff")}>
                  <table style={S("width:100%;min-width:660px;border-collapse:collapse;font-size:13px;table-layout:fixed")}>
                    <thead>
                      <tr>
                        <th style={S("background:#F7F9F8;color:#7A808B;font-size:11.5px;font-weight:700;text-align:left;padding:14px 18px;white-space:nowrap;border-bottom:1px solid #E7EAEF;width:150px")}>구분</th>
                    ...
                    <tbody>
                      {[
                        ...
                        { label: '경증 사고 보장', sub: '−1.75σ 이탈 시', key: 'mildLabel' },
              ...
              <div style={S("max-width:1160px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>KEY POINTS</div>
                ...
                <div style={S("display:flex;gap:16px;margin-top:26px;flex-wrap:wrap")}>
                  {vm.keyPoints.map((k, ki) => (
                    <React.Fragment key={ki}>
                      <div className="hov-lift" style={S("flex:1 1 280px;min-width:0;background:#fff;border:1px solid #A9D3BC;border-radius:20px;padding:clamp(20px,2.6vw,26px);display:flex;flex-direction:column;box-shadow:0 6px 18px rgba(0,70,42,0.06)")}>
                        <span style={S("width:44px;height:44px;border-radius:13px;flex:none;background:#EAF5EF;border:1px solid #CDE7D9;display:flex;align-items:center;justify-content:center")}>{this.uiIcon(k.icon, 23, '#0C6742')}</span>
                        <div style={S("font-size:16px;font-weight:900;color:#00462A;margin-top:14px;letter-spacing:-0.4px")}>{k.title}</div>
                        <div style={S("font-size:13.5px;font-weight:700;color:#0C6742;margin-top:8px;line-height:1.65;word-break:keep-all")}>{k.lead}</div>
                        {k.points ? (
                          <div style={S("margin-top:13px;display:flex;flex-direction:column;gap:9px;flex:1")}>
                            {k.points.map((pt, pj) => (
                              <div key={pj} style={S("display:flex;gap:9px;align-items:flex-start")}>
                                <span style={S("flex:none;width:19px;height:19px;border-radius:50%;background:#0C6742;color:#fff;font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;margin-top:1px")}>{pj + 1}</span>
              ...
              <div style={S("max-width:900px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                ...
                <div style={S("font-size:11.5px;color:#9AA0AB;margin-top:14px;line-height:1.7;word-break:keep-all")}>
                  ...
                  <a href={vm.policyUrl + '#' + vm.anchors.지급제한} target="_blank" rel="noreferrer" style={S("color:#0C6742;font-weight:700")}>전체 지급제한사항 보기 ↗</a>
                </div>
              </div>
              ...
              <div style={S("max-width:820px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>FAQ</div>
              ...
              <div style={S("max-width:860px;margin:0 auto;padding:clamp(36px,5vw,52px) 20px 20px;width:100%;box-sizing:border-box")}>
                <div style={S("text-align:center")}>
                  <div style={S("font-size:13px;font-weight:800;color:#0C6742")}>APPLICATION</div>
                ...
                <div style={S("margin-top:24px;background:#F7FBF9;border:1px solid #C9E1D4;border-radius:20px;padding:clamp(14px,2vw,20px);display:flex;flex-direction:column")}>
                  {vm.applyPrep.map((p, pi) => (
                    <React.Fragment key={pi}>
                      <div style={S(`display:flex;gap:14px;align-items:flex-start;padding:14px 6px;border-top:${pi === 0 ? '0' : '1px solid #E2EFE8'}`)}>
                        <span style={S("width:26px;height:26px;border-radius:50%;flex:none;display:flex;align-items:center;justify-content:center;font-size:12.5px;font-weight:900;background:#fff;border:1px solid #A9D3BC;color:#0C6742;margin-top:1px")}>{p.no}</span>
                        <div style={S("flex:1;min-width:0")}>
                ...
                <div style={S("font-size:12.5px;font-weight:800;color:#0C6742;margin-top:22px;margin-bottom:2px")}>약관·상품설명서 원문</div>
                ...
                <div style={S("margin-top:16px")}>
                  <a className="hov-btn" href={vm.policyUrl} target="_blank" rel="noreferrer"
                     style={S("text-decoration:none;background:#fff;border:1.5px solid #A9D3BC;color:#00462A;border-radius:14px;padding:14px 18px;font-size:13.5px;font-weight:800;display:flex;align-items:center;justify-content:center;gap:8px;word-break:keep-all")}>
                    <span style={S("flex:none;display:flex")}>{this.uiIcon('book', 18, '#0C6742')}</span>약관·상품설명서 전문 보기 <span style={S("font-size:11px;color:#6E8A7C;flex:none")}>↗</span>
                  </a>
                </div>

                <div style={S("display:flex;gap:10px;margin-top:18px;flex-wrap:wrap;align-items:stretch")}>
                  <a className="hov-btn" href={vm.applicationUrl} target="_blank" rel="noreferrer"
                     style={S("flex:1 1 200px;min-width:0;text-decoration:none;background:#fff;border:1.5px solid #A9D3BC;color:#00462A;border-radius:14px;padding:16px 24px;font-size:13.5px;font-weight:800;display:flex;align-items:center;justify-content:center;gap:8px;word-break:keep-all")}>
                    빈 청약서 양식 보기 <span style={S("font-size:11px;color:#6E8A7C;flex:none")}>↗</span>
                  </a>
                  <div className="hov-btn" role="button" tabIndex={0}
                     style={S("flex:1 1 240px;min-width:0;background:linear-gradient(135deg,#00462A,#0A5F3C);color:#fff;border-radius:14px;padding:16px 24px;font-size:14.5px;font-weight:900;display:flex;align-items:center;justify-content:center;cursor:pointer;word-break:keep-all")}
                ...
                <div style={S("font-size:11.5px;color:#8FA79A;margin-top:10px;line-height:1.7;text-align:center;word-break:keep-all")}>
                  빈 양식은 작성 전 확인용이며, 실제 가입은 약관·상품설명 확인 후 청약서 작성으로 이어집니다. 피보험자가 미성년자인 경우 법정대리인의 동의와 서명이 필요합니다.
                </div>
            ...
            <div style={S(['apply','done'].includes(vm.entry)
             ...
             <div style={S(['apply','done'].includes(vm.entry)
              ...
              <div style={S("position:sticky;top:0;height:52px;box-sizing:border-box;z-index:3;display:flex;align-items:center;gap:8px;padding:0 14px;background:#fff;border-bottom:1px solid #E6E6E6")}>
                <span style={S("font-size:19px;color:#555;cursor:pointer")} onClick={() => vm.entryGo(vm.entry === 'apply' ? 'terms' : (vm.payBackEntry || 'landing'))}>{vm.entry === 'apply' ? '‹' : '✕'}</span>
                    ...
                    <div style={S("display:flex;align-items:flex-start;gap:10px")}>
                      ...
                      <div style={S("flex:1;min-width:0")}>
                        <div style={S("display:flex;align-items:flex-start;justify-content:space-between;gap:10px")}>
                          <div>
                            <div style={S("font-size:13px;font-weight:800;color:#111;line-height:1.35")}>재수없수 학습성취 보장보험 함께 가입</div>
                            <div style={S("font-size:10.5px;color:#0B8F58;font-weight:700;margin-top:3px")}>월 2,000원대부터 · 비성적 객관 지표 기반 개인 산정</div>
                          </div>
                      ...
                      <div style={S(`border-radius:20px;padding:15px 16px;background:#fff;border:1.5px solid ${vm.onbForm.tier === t.tier ? '#0B8F58' : '#E6E6E6'}`)}>
                        ...
                        {(vm.expandedTier === t.tier) && (<>
                          <div style={S("background:#F8FAF9;border-radius:14px;padding:12px 13px;margin-top:8px;display:flex;flex-direction:column;gap:9px;animation:riseIn .25s ease")}>
                            {[['🟡','경증 사고','수능 성적이 예측 밴드 하단(−1.75σ)을 벗어난 경우',`최대 ${(t.cover_mild/10000).toLocaleString('ko-KR')}만원 · 재수비용 6개월분(70%)`],['🔴','중증 사고','−2.25σ 이하로 크게 하락한 경우',`최대 ${(t.cover_sev/10000).toLocaleString('ko-KR')}만원 · 재수비용 12개월분(70%)`],['💳','지급 방식','보통약관은 보험가입금액 정액 지급','실제 수강료 실비 보상은 재수학원비 지원 특약(선택)'],['📅','보장 조건','수능 1회 고정 · 별도 면책 대기기간 없음','재수(재응시) 실행이 확인되어야 지급']].map((r,ri)=>(
                              <React.Fragment key={ri}>
                ...
                {(vm.entry === 'apply') && (<>
                  ...
                  <div style={S("background:#fff;border:1px solid #DADADA;border-radius:6px;padding:0;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.05)")}>
                    ...
                    <div style={S("padding:14px 15px;display:flex;flex-direction:column;gap:16px")}>
                      ...
                      {/* 가입 상품 선택 */}
                      <div>
                        <div style={S("font-size:11.5px;font-weight:800;color:#0B7A4A;border-bottom:2px solid #0B7A4A;padding-bottom:4px")}>가입 상품 선택</div>
                        <div style={S("display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin-top:9px")}>
                          {vm.tierRows.map((t, ti) => (
                      ...
                      <div>
                        ...
                        <div style={S("display:flex;gap:6px;flex-wrap:wrap")}>
                          {vm.regionOptions.map((o,oi)=>(<div key={oi} onClick={()=>vm.setApply({region:o})} style={S(`font-size:10px;font-weight:600;padding:7px 11px;border-radius:8px;cursor:pointer;border:1.5px solid ${vm.apply.region===o?'#0B7A4A':'#DDD'};background:${vm.apply.region===o?'#0B7A4A':'#fff'};color:${vm.apply.region===o?'#fff':'#555'}`)}>{o}</div>))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        {[['현재 학년','q_grade',['고1','고2','고3(6월 모평 전)'],'고3 6월 모의평가 이후 신규가입 불가'],['대입 준비 방향 (참고용)','q_direction',['정시 중심','수시·정시 병행','수시 중심'],'요율·사고 판정·가입 제한에 미반영 — 진학 상담 참고용'],['성적자료 제출 동의','q_data',['동의','미동의'],'미동의 시 가입 불가']].map((q,qi)=>(
                          <React.Fragment key={qi}>
                            <div style={S("margin-top:9px")}>
                              ...
                              <div style={S(`font-size:9px;color:${q[1] === 'q_data' ? '#C0304A' : '#999'};font-weight:${q[1] === 'q_data' ? '800' : '400'};margin-top:3px`)}>{q[3]}</div>
                            </div>
                          </React.Fragment>
                        ))}
                      ...
                      {/* Ⅳ 요율 산출 문항 */}
                      <div>
                        <div style={S("font-size:11.5px;font-weight:800;color:#0B7A4A;border-bottom:2px solid #0B7A4A;padding-bottom:4px")}>Ⅳ. 보험료 산출 문항 <span style={S("font-size:8.5px;color:#0B8F58")}>요율 반영 — 이 3개 항목만 사용</span></div>
                      ...
                      <div>
                        <div style={S("font-size:11.5px;font-weight:800;color:#0B7A4A;border-bottom:2px solid #0B7A4A;padding-bottom:4px")}>Ⅴ. 포트폴리오·통계 분석용 참고 문항 <span style={S("font-size:8.5px;color:#999")}>요율 미반영</span></div>
                      ...
                      {/* Ⅵ 개인정보 동의 */}
                      <div>
                        <div style={S("font-size:11.5px;font-weight:800;color:#0B7A4A;border-bottom:2px solid #0B7A4A;padding-bottom:4px")}>Ⅵ. 개인정보 수집·이용 동의</div>
                      ...
                      {/* Ⅶ 자필서명 */}
                      <div>
                        <div style={S("font-size:11.5px;font-weight:800;color:#0B7A4A;border-bottom:2px solid #0B7A4A;padding-bottom:4px")}>Ⅶ. 확인 및 자필서명</div>
                        ...
                        <div style={S(`display:flex;align-items:center;gap:10px;margin-top:10px;cursor:pointer;background:${vm.apply.sign?'rgba(11,143,88,0.06)':'#FAFAFA'};border:1.5px solid ${vm.apply.sign?'#0B8F58':'#DDD'};border-radius:14px;padding:12px 13px`)} onClick={()=>vm.setApply({sign:!vm.apply.sign})}>
                          ...
                          <span style={S("font-size:10.5px;color:#333;line-height:1.4")}>본인은 위 청약 내용을 확인하고 <b style={S("color:#0B8F58")}>전자서명에 동의</b>합니다. 전자서명자: {vm.apply.p_name||vm.onbForm.name||'피보험자'}</span>
                        </div>
                      </div>
                          ...
                          <div style={S("display:flex;gap:8px;margin-top:12px;flex-wrap:wrap")}>
                            {[['경증 사고 시', vm.enrolledQuote.coverage.mild], ['중증 사고 시', vm.enrolledQuote.coverage.severe]].map((r, ri) => (
                              <div key={ri} style={S("flex:1 1 140px;background:#fff;border:1px solid #DCEBE3;border-radius:12px;padding:9px 11px")}>
                                ...
                                <div style={S("font-size:14px;font-weight:900;color:#0C6742;margin-top:2px")}>{(r[1] / 10000).toLocaleString('ko-KR')}만원</div>
                              </div>
                            ))}
                          </div>
                        )}
